...
class ArcFace {
  ...
  constructor(env) {
    ...
    const {
      ...
      ARCSOFT_FD_SDKKEY,
      ARCSOFT_FR_SDKKEY,
      FACE_DETECTION_LIB_PATH = require.resolve(
        '../libs/libarcsoft_fsdk_face_detection.so',
      ),
      MAX_FACE = 50,
      FACE_RECOGNITION_LIB_PATH = require.resolve(
        '../libs/libarcsoft_fsdk_face_recognition.so',
      ),
    } = env;
    this.initialFDEngine(FACE_DETECTION_LIB_PATH, {
      appId: ARCSOFT_APP_ID,
      sdkKey: ARCSOFT_FD_SDKKEY,
      maxFace: MAX_FACE,
    });
    this.initialFREngine(FACE_RECOGNITION_LIB_PATH, {
      appId: ARCSOFT_APP_ID,
  ...
  uninitialFREngine() {
    ...
    if (!!this.pFREngine && !this.hFREngine.isNull()) {
      ...
      if (nRet !== 0) {
        throw new Error(
          'Uninitial Face Recognition Engine failed, code ' + nRet,
        );
      }
    }
  }
  ...
  extractFeature(asvl, rcFace, orient) {
    ...
    const nRet = this.FR.AFR_FSDK_ExtractFRFeature(
      this.hFREngine,
      asvl.ref(),
      faceInput.ref(),
      faceModel.ref(),
    ...
    // copy
    libc.memcpy(
      _faceModel.pbFeature.address(),
      faceModel.pbFeature.address(),
      _faceModel.lFeatureSize,
    );
  ...
  compare(faceModel1, faceModel2) {
    ...
    const nRet = this.FR.AFR_FSDK_FacePairMatching(
      this.hFREngine,
      faceModel1.ref(),
      faceModel2.ref(),
      pSimil,
  ...
  detect(asvl) {
    ...
    const nRet = this.FD.AFD_FSDK_StillImageFaceDetection(
      this.pFDEngine.deref(),
      asvl.ref(),
      pFaceRes,
    );
    ...
    for (let i = 0; i < faceRes.nFace; ++i) {
      libc.memcpy(
        rect.ref().address(),
        faceRes.rcFace.address() + i * TypeDef.MRECT.size,
        TypeDef.MRECT.size,
      );
      libc.memcpy(
        orientBuf.address(),
        faceRes.lfaceOrient.address() + i * TypeDef.AFD_FSDK_OrientCode.size,
        TypeDef.AFD_FSDK_OrientCode.size,
      );
